...
<Query>
<![CDATA[select c.地区,数量,利润,利润/总利润 as 利润贡献率 from (select a.地区,数量,利润 from (SELECT S客户.地区,count(S客户.公司名称) as 数量
FROM S客户
group by S客户.地区) as a
inner join 
(SELECT S客户.地区,sum((S订单明细.单价-S订单明细.进价)*S订单明细.数量*(1-S订单明细.折扣)-S成本记录表.ShQJE) as 利润
FROM S订单,S客户,S订单明细,S成本记录表
where S订单.客户ID = S客户.客户ID
and S订单.订单ID = S订单明细.订单ID
and S订单.雇员ID = S成本记录表.ShQR
group  by S客户.地区) as b
on a.地区=b.地区) as c
inner join
(SELECT S客户.地区,(select sum((S订单明细.单价-S订单明细.进价)*S订单明细.数量*(1-S订单明细.折扣)-S成本记录表.ShQJE) from S订单,S客户,S订单明细,S成本记录表 where S订单.客户ID = S客户.客户ID and S订单.订单ID = S订单明细.订单ID and S订单.雇员ID = S成本记录表.ShQR) as 总利润
FROM S订单,S客户,S订单明细,S成本记录表
where S订单.客户ID = S客户.客户ID
and S订单.订单ID = S订单明细.订单ID
and S订单.雇员ID = S成本记录表.ShQR
group by S客户.地区) as d
on c.地区=d.地区]]></Query>
</TableData>
...
<![CDATA[SELECT 
     S客户.地区,
     S客户.公司名称,
    (sum(S订单明细.数量*S订单明细.单价*(1-S订单明细.折扣))) as 订单总额
FROM
     S客户,S订单明细,S订单
where
 S客户.地区='${P1}'
and  
     S订单明细.订单ID = S订单.订单ID 
and  S订单.客户ID = S客户.客户ID
group  by S客户.地区,S客户.公司名称
order by sum(S订单明细.数量*S订单明细.单价*(1-S订单明细.折扣)) desc limit 6]]></Query>
</TableData>
...
<![CDATA[SELECT
     S客户.地区,
     S客户.公司名称,
     strftime("%Y-%m-%d",S订单.订购日期) as 订购日期,
    (sum(S订单明细.单价*S订单明细.数量*(1-S订单明细.折扣))) as 订单金额
FROM
     S订单明细,
     S订单,
     S客户
...
GROUP BY
     S客户.地区,
     S客户.公司名称,
     strftime("%Y-%m-%d",S订单.订购日期)]]></Query>
</TableData>
<TableData name="季节变动" class="com.fr.data.impl.DBTableData">
<Parameters>
<Parameter>
<Attributes name="P2"/>
<O>
...
<Query>
<![CDATA[select * from (SELECT round(strftime('%m',S订单.订购日期)/3+0.49999999) as 季度, strftime('%Y',S订单.订购日期) as 年,sum(S订单明细.单价*S订单明细.数量*(1-S订单明细.折扣)) as 销售额 
FROM S订单明细,S订单,S客户
where S订单明细.订单ID=S订单.订单ID and S订单.客户ID = S客户.客户ID and  S客户.公司名称='${P2}'
group by round(strftime('%m',S订单.订购日期)/3+0.49999999),strftime('%Y',S订单.订购日期)
order by  round(strftime('%m',S订单.订购日期)/3+0.49999999)) a


]]></Query>
</TableData>
<TableData name="季节变动1" class="com.fr.data.impl.DBTableData">
<Parameters>
<Parameter>
<Attributes name="P2"/>
<O>
...
<Query>
<![CDATA[
SELECT round(strftime('%m',S订单.订购日期)/3+0.49999999) as 季度, '2013' as 年,sum(S订单明细.单价*S订单明细.数量*(1-S订单明细.折扣))/3*(1+0.08) as 销售额 
FROM S订单明细,S订单,S客户
where S订单明细.订单ID=S订单.订单ID and S订单.客户ID = S客户.客户ID and  S客户.公司名称='${P2}'
group by round(strftime('%m',S订单.订购日期)/3+0.49999999),'2013']]></Query>
</TableData>
</TableDataMap>
<Parameters/>
...
<ChartDefinition>
<OneValueCDDefinition seriesName="公司名称" valueName="订单金额" function="com.fr.data.util.function.SumFunction">
...
<BoundsAttr x="0" y="36" width="680" height="439"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
...
<BoundsAttr x="0" y="0" width="680" height="36"/>
</Widget>
...
<BoundsAttr x="0" y="1008" width="680" height="504"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<DataProcessor class="com.fr.base.chart.chartdata.model.NormalDataModel"/>
<ChartDefinition>
<OneValueCDDefinition seriesName="地区" valueName="订单总额" function="com.fr.data.util.function.SumFunction">
<Top topCate="-1" topValue="-1" isDiscardOtherCate="false" isDiscardOtherSeries="false" isDiscardNullCate="false" isDiscardNullSeries="false"/>
<TableData class="com.fr.data.impl.NameTableData">
<Name>
<![CDATA[ds2]]></Name>
...
<BoundsAttr x="0" y="36" width="680" height="438"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.Label">
...
<BoundsAttr x="0" y="504" width="680" height="504"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<CategoryName value="c.地区"/>
<ChartSummaryColumn name="利润贡献率" function="com.fr.data.util.function.NoneFunction" customName="利润贡献率"/>
</MoreNameCDDefinition>
</DefinitionMap>
...
<BoundsAttr x="0" y="0" width="680" height="504"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<VanChartColumnPlotAttr seriesOverlapPercent="20.0" categoryIntervalPercent="20.0" fixedWidth="false" columnWidth="20" filledWithImage="false" isBar="false"/>
</VanChartPlot>
<VanChartPlot class="com.fr.plugin.chart.line.VanChartLinePlot">
<GI>
<AttrBackground>
...
<DefinitionMapList>
<DefinitionMap key="line">
<OneValueCDDefinition seriesName="年" valueName="销售额" function="com.fr.data.util.function.SumFunction">
<Top topCate="-1" topValue="-1" isDiscardOtherCate="false" isDiscardOtherSeries="false" isDiscardNullCate="false" isDiscardNullSeries="false"/>
<TableData class="com.fr.data.impl.NameTableData">
<Name>
<![CDATA[季节变动1]]></Name>
...
<BoundsAttr x="0" y="1512" width="680" height="508"/>
</Widget>
<Sorted sorted="false"/>
<WidgetZoomAttr compState="1"/>
<AppRelayout appRelayout="false"/>
<Size width="680" height="2020"/>
<MobileWidgetList/>
</Center>
</Layout>
